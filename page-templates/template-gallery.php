...
<section style="background: #F5F6FA; padding: 40px 0 0; position: relative; margin-top: 0; margin-bottom: 0;">
  <div class="inner">
    ...
    <!-- Results Count -->
    <div style="display: flex; align-items: center; justify-content: flex-end; margin-bottom: 0;">
      <p id="gallery-count" style="font-size: 15px; color: #6B7280; font-weight: 500; margin: 0;">
        Showing <span id="count-number">0</span> photos
      </p>
    </div>
  </div>
</section>
...
function updateCount() {
  ...
  const visibleItems = Array.from(photosGrid.querySelectorAll('.gallery-item')).filter(item => {
    return item.style.display !== 'none';
  });
